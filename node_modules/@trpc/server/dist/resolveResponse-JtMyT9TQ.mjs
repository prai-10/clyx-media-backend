import { c as isAsyncIterable, i as abortSignalsAnyPonyfill, l as isFunction, o as emptyObject, p as run, s as identity, u as isObject$1 } from "./codes-D5Ya6_Bi.mjs";
import { n as _objectSpread2, o as getHTTPStatusCode, r as _defineProperty, t as getErrorShape } from "./getErrorShape-B0JBUs-i.mjs";
import { g as getTRPCErrorFromUnknown, m as TRPCError, p as transformTRPCResponse, s as getProcedureAtPath, t as isTrackedEnvelope } from "./tracked-D4jU_Hb3.mjs";
import { r as observableToAsyncIterable, t as isObservable } from "./observable-DwUFSuKv.mjs";
//#region src/unstable-core-do-not-import/http/parseConnectionParams.ts
function parseConnectionParamsFromUnknown(parsed) {
	try {
		if (parsed === null) return null;
		if (!isObject$1(parsed)) throw new Error("Expected object");
...
const jsonContentTypeHandler = {
	...
	async parse(opts) {
		...
		const getInputs = memo(async () => {
			...
			if (!isObject$1(inputs)) throw new TRPCError({
				code: "BAD_REQUEST",
			...
			return {
				...
				getRawInput: async () => {
					...
					if ((procedure === null || procedure === void 0 ? void 0 : procedure._def.type) === "subscription") {
						...
						if (lastEventId) {
							if (isObject$1(input)) input = _objectSpread2(_objectSpread2({}, input), {}, { lastEventId });
							else {
								var _input;
...
function isAbortError(error) {
	return isObject$1(error) && error["name"] === "AbortError";
}
...
var Unpromise = class Unpromise {
	constructor(arg) {
		...
		_defineProperty(this, "settlement", null);
		_defineProperty(this, _Symbol$toStringTag, "Unpromise");
...
function makeResource(thing, dispose) {
	...
	it[Symbol.dispose] = () => {
		dispose();
		existing === null || existing === void 0 || existing.call(it);
	};
	return it;
}
/**
...
function makeAsyncResource(thing, dispose) {
	...
	it[Symbol.asyncDispose] = async () => {
		await dispose();
		await (existing === null || existing === void 0 ? void 0 : existing.call(it));
	};
	return it;
}
//#endregion
...
function _takeWithGrace() {
	_takeWithGrace = _wrapAsyncGenerator(function* (iterable, opts) {
		try {
			var _usingCtx$5 = _usingCtx();
			const iterator = _usingCtx$5.a(iteratorResource(iterable));
			let result;
			const timer = _usingCtx$5.u(timerResource(opts.gracePeriodMs));
			let count = opts.count;
			let timerPromise = new Promise(() => {});
			while (true) {
				result = yield _awaitAsyncGenerator(Unpromise.race([iterator.next(), timerPromise]));
			...
			yield _awaitAsyncGenerator(_usingCtx$5.d());
		}
	});
	return _takeWithGrace.apply(this, arguments);
}
		...
		add(iterable) {
			switch (state) {
				case "idle":
					...
					break;
				case "pending": initIterable(iterable);
			}
		},
		[Symbol.asyncIterator]() {
			return _wrapAsyncGenerator(function* () {
				try {
					...
					state = "pending";
					_usingCtx$4.a(makeAsyncResource({}, async () => {
						state = "done";
						const errors = [];
					...
					while (iterators.size > 0) {
						yield _awaitAsyncGenerator(flushSignal.promise);
					...
					yield _awaitAsyncGenerator(_usingCtx$4.d());
				}
			})();
		}
	};
...
function _withPing() {
	_withPing = _wrapAsyncGenerator(function* (iterable, pingIntervalMs) {
		try {
			var _usingCtx$3 = _usingCtx();
			const iterator = _usingCtx$3.a(iteratorResource(iterable));
			let result;
			let nextPromise = iterator.next();
			while (true) try {
				var _usingCtx3 = _usingCtx();
				const pingPromise = _usingCtx3.u(timerResource(pingIntervalMs));
				result = yield _awaitAsyncGenerator(Unpromise.race([nextPromise, pingPromise.start()]));
			...
			yield _awaitAsyncGenerator(_usingCtx$3.d());
		}
	});
	return _withPing.apply(this, arguments);
}
...
function isPromise(value) {
	return (isObject$1(value) || isFunction(value)) && typeof (value === null || value === void 0 ? void 0 : value["then"]) === "function" && typeof (value === null || value === void 0 ? void 0 : value["catch"]) === "function";
}
...
function _createBatchStreamProducer() {
	_createBatchStreamProducer = _wrapAsyncGenerator(function* (opts) {
		const { data } = opts;
		let counter = 0;
		const placeholder = 0;
		...
		function encodePromise(promise, path) {
			return registerAsync(/* @__PURE__ */ function() {
				var _ref = _wrapAsyncGenerator(function* (idx) {
					...
					try {
						const next = yield _awaitAsyncGenerator(promise);
						yield [
							idx,
		...
		function encodeAsyncIterable(iterable, path) {
			return registerAsync(/* @__PURE__ */ function() {
				var _ref2 = _wrapAsyncGenerator(function* (idx) {
					try {
						var _usingCtx$2 = _usingCtx();
						const error = checkMaxDepth(path);
						if (error) throw error;
						const iterator = _usingCtx$2.a(iteratorResource(iterable));
						try {
							while (true) {
								const next = yield _awaitAsyncGenerator(iterator.next());
								if (next.done) {
									yield [
										idx,
						...
						yield _awaitAsyncGenerator(_usingCtx$2.d());
					}
				});
				return function(_x2) {
		...
		var _iteratorError;
		try {
			for (var _iterator = _asyncIterator(iterable), _step; _iteratorAbruptCompletion = !(_step = yield _awaitAsyncGenerator(_iterator.next())).done; _iteratorAbruptCompletion = false) yield _step.value;
		} catch (err) {
...
async function jsonlStreamConsumer(opts) {
	...
	function decodeChunkDefinition(value) {
		...
		switch (type) {
			case CHUNK_VALUE_TYPE_PROMISE: return run(async () => {
				try {
					var _usingCtx3 = _usingCtx();
					const { value } = await _usingCtx3.u(controller.getReaderResource()).read();
			...
			case CHUNK_VALUE_TYPE_ASYNC_ITERABLE: return run(_wrapAsyncGenerator(function* () {
				try {
					var _usingCtx4 = _usingCtx();
					const reader = _usingCtx4.u(controller.getReaderResource());
					while (true) {
						const { value } = yield _awaitAsyncGenerator(reader.read());
						const [_chunkId, status, data] = value;
						switch (status) {
	...
	const handleClose = () => {
		if (headDeferred) {
			headDeferred.reject(/* @__PURE__ */ new Error("Stream closed before head was received"));
			headDeferred = null;
		}
	...
	source.pipeTo(new WritableStream({
		write(chunkOrHead) {
			if (headDeferred) {
				...
				for (const [key, value] of Object.entries(chunkOrHead)) head[key] = decode(value);
			...
			const [idx] = chunk;
			streamManager.getOrCreate(idx).enqueue(chunk);
		},
		close: handleClose,
		abort: handleAbort
	})).catch((error) => {
...
//#endregion
//#region \0@oxc-project+runtime@0.149.0/helpers/esm/asyncGeneratorDelegate.js
...
function sseStreamProducer(opts) {
	...
	function _generatorWithErrorHandling() {
		_generatorWithErrorHandling = _wrapAsyncGenerator(function* () {
			try {
				yield* _asyncGeneratorDelegate(_asyncIterator(generator()));
				yield {
					event: RETURN_EVENT,
					data: ""
				};
...
function sseStreamConsumer(opts) {
	...
	const createStream = () => new ReadableStream({
		async start(controller) {
			...
			eventSource.addEventListener(CONNECTED_EVENT, (_msg) => {
				const options = JSON.parse(_msg.data);
			...
			eventSource.addEventListener("message", (_msg) => {
				const msg = _msg;
				const def = { data: deserialize(JSON.parse(msg.data)) };
	...
	return run(_wrapAsyncGenerator(function* () {
		try {
			var _usingCtx3 = _usingCtx();
			...
			while (true) {
				...
				const result = yield _awaitAsyncGenerator(promise);
				if (result.done) return result.value;
				yield result.value;
			}
		} catch (_) {
	...
	*/
	...
	try {
		...
		if (!info.isBatchCall) {
			...
			switch (info.type) {
				...
				case "subscription": {
					...
					const stream = sseStreamProducer(_objectSpread2(_objectSpread2({}, config.sse), {}, {
						data: iterable,
		...
		if (info.accept === "application/jsonl") {
			...
			const stream = jsonlStreamProducer(_objectSpread2(_objectSpread2({}, config.jsonl), {}, {
				/**
...
//#endregion
export { getAcceptHeader as _, isPromise as a, parseConnectionParamsFromUnknown as b, createDeferred as c, _usingCtx as d, makeAsyncResource as f, throwAbortError as g, isAbortError as h, sseStreamProducer as i, iteratorResource as l, Unpromise as m, sseHeaders as n, jsonlStreamConsumer as o, makeResource as p, sseStreamConsumer as r, jsonlStreamProducer as s, resolveResponse as t, takeWithGrace as u, getRequestInfo as v, parseConnectionParamsFromString as y };

//# sourceMappingURL=resolveResponse-JtMyT9TQ.mjs.map