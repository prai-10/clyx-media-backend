...
var Unpromise = class Unpromise {
	constructor(arg) {
		require_getErrorShape._defineProperty(this, "promise", void 0);
		require_getErrorShape._defineProperty(this, "subscribers", []);
		require_getErrorShape._defineProperty(this, "settlement", null);
		require_getErrorShape._defineProperty(this, _Symbol$toStringTag, "Unpromise");
...
async function jsonlStreamConsumer(opts) {
	...
	function decodeChunkDefinition(value) {
		...
		switch (type) {
			...
			case CHUNK_VALUE_TYPE_ASYNC_ITERABLE: return require_codes.run(_wrapAsyncGenerator(function* () {
				try {
...
function sseStreamConsumer(opts) {
	...
	return require_codes.run(_wrapAsyncGenerator(function* () {
		try {
			var _usingCtx3 = _usingCtx();
...
function combinedAbortController(signal) {
	...
	return {
		signal: require_codes.abortSignalsAnyPonyfill([signal, controller.signal]),
		controller
	};
}
	...
	*/
	...
	try {
			...
			switch (info.type) {
				...
				case "subscription": {
					const iterable = require_codes.run(() => {
						...
						return require_observable.isObservable(result.data) ? require_observable.observableToAsyncIterable(result.data, opts.req.signal) : result.data;
					});
					const stream = sseStreamProducer(require_getErrorShape._objectSpread2(require_getErrorShape._objectSpread2({}, config.sse), {}, {
						data: iterable,
						...
						formatError(errorOpts) {
							...
							return require_getErrorShape.getErrorShape({
								config,
								ctx: ctxManager.valueOrUndefined(),
								error,
								input,
		...
		if (info.accept === "application/jsonl") {
			...
			const stream = jsonlStreamProducer(require_getErrorShape._objectSpread2(require_getErrorShape._objectSpread2({}, config.jsonl), {}, {
				/**
