...
function createInputMiddleware(parse) {
	const inputMiddleware = async function inputValidatorMiddleware(opts) {
		...
		const combinedInput = require_codes.isObject(opts.input) && require_codes.isObject(parsedInput) ? require_getErrorShape._objectSpread2(require_getErrorShape._objectSpread2({}, opts.input), parsedInput) : parsedInput;
...
function createOutputMiddleware(parse) {
	const outputMiddleware = async function outputValidatorMiddleware({ next }) {
		...
		try {
			...
			return require_getErrorShape._objectSpread2(require_getErrorShape._objectSpread2({}, result), {}, { data });
		} catch (cause) {
			throw new require_tracked.TRPCError({
...
var StandardSchemaV1Error = class extends Error {
	...
	*/
	constructor(issues) {
		...
		require_getErrorShape._defineProperty(this, "issues", void 0);
...
function createNewBuilder(def1, def2) {
	...
	return createBuilder(require_getErrorShape._objectSpread2(require_getErrorShape._objectSpread2({}, require_codes.mergeWithoutOverrides(def1, rest)), {}, {
		inputs: [...def1.inputs, ...inputs !== null && inputs !== void 0 ? inputs : []],
		middlewares: [...def1.middlewares, ...middlewares],
		meta: def1.meta && meta ? require_getErrorShape._objectSpread2(require_getErrorShape._objectSpread2({}, def1.meta), meta) : meta !== null && meta !== void 0 ? meta : def1.meta
	}));
}
function createBuilder(initDef = {}) {
	const _def = require_getErrorShape._objectSpread2({
		procedure: true,
		inputs: [],
		middlewares: []
	}, initDef);
	return {
		...
		query(resolver) {
			return createResolver(require_getErrorShape._objectSpread2(require_getErrorShape._objectSpread2({}, _def), {}, { type: "query" }), resolver);
		},
		mutation(resolver) {
			return createResolver(require_getErrorShape._objectSpread2(require_getErrorShape._objectSpread2({}, _def), {}, { type: "mutation" }), resolver);
		},
		subscription(resolver) {
			return createResolver(require_getErrorShape._objectSpread2(require_getErrorShape._objectSpread2({}, _def), {}, { type: "subscription" }), resolver);
		},
...
function createResolver(_defIn, resolver) {
	...
	const _def = require_getErrorShape._objectSpread2(require_getErrorShape._objectSpread2({}, finalBuilder._def), {}, {
		type: _defIn.type,
...
async function callRecursive(index, _def, opts) {
	try {
		...
		return await middleware(require_getErrorShape._objectSpread2(require_getErrorShape._objectSpread2({}, opts), {}, {
			meta: _def.meta,
			input: opts.input,
			next(_nextOpts) {
				var _nextOpts$getRawInput;
				const nextOpts = _nextOpts;
				return callRecursive(index + 1, _def, require_getErrorShape._objectSpread2(require_getErrorShape._objectSpread2({}, opts), {}, {
					ctx: (nextOpts === null || nextOpts === void 0 ? void 0 : nextOpts.ctx) ? require_getErrorShape._objectSpread2(require_getErrorShape._objectSpread2({}, opts.ctx), nextOpts.ctx) : opts.ctx,
...
const initTRPC = new class TRPCBuilder {
	...
	*/
	create(opts) {
		...
		const config = require_getErrorShape._objectSpread2(require_getErrorShape._objectSpread2({}, opts), {}, {
