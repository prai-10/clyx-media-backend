...
var TRPCError = class extends Error {
	constructor(opts) {
		...
		super(message, { cause });
		require_getErrorShape._defineProperty(this, "cause", void 0);
		require_getErrorShape._defineProperty(this, "code", void 0);
		this.code = opts.code;
		this.name = "TRPCError";
...
function transformTRPCResponseItem(config, item) {
	if ("error" in item) return require_getErrorShape._objectSpread2(require_getErrorShape._objectSpread2({}, item), {}, { error: config.transformer.output.serialize(item.error) });
	if ("data" in item.result) return require_getErrorShape._objectSpread2(require_getErrorShape._objectSpread2({}, item), {}, { result: require_getErrorShape._objectSpread2(require_getErrorShape._objectSpread2({}, item.result), {}, { data: config.transformer.output.serialize(item.result.data) }) });
	return item;
}
/**
...
function transformResultInner(response, transformer) {
	if ("error" in response) {
		...
		return {
			ok: false,
			error: require_getErrorShape._objectSpread2(require_getErrorShape._objectSpread2({}, response), {}, { error })
		};
	}
	return {
		ok: true,
		result: require_getErrorShape._objectSpread2(require_getErrorShape._objectSpread2({}, response.result), (!response.result.type || response.result.type === "data") && {
			type: "data",
...
function createRouterFactory(config) {
	function createRouterInner(input) {
		...
		const lazy = require_codes.emptyObject();
		...
		const _def = require_getErrorShape._objectSpread2(require_getErrorShape._objectSpread2({
			_config: config,
			router: true,
			procedures,
			lazy
		}, emptyRouter), {}, { record });
		return require_getErrorShape._objectSpread2(require_getErrorShape._objectSpread2({}, record), {}, {
			_def,
